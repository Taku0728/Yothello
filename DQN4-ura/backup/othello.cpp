...
double AB_value(Board &b, const Neuralnet &neuralnet, const int disks, const int depth, double alp, double bet, const double cut_ratio)
{
	...
	for (auto &next_b : next_bs) {
		double value = -AB_value(next_b, neuralnet, disks + 1, depth - 1, -bet, -alp, cut_ratio);
		alp = max(alp, value);
		if (alp >= bet) {
			return alp;
		}
...
double AB_value(Board &b, const Neuralnet &neuralnet, const int disks, const int depth, double alp, double bet, const double cut_ratio, vector<pair<int, int> > &moves)
{
	int inputs(neuralnet.nodes[0]);
	if (disks == 64) {
		b.calculate_disks();
	...
	if (move_number == 0) {
		...
		moves.emplace_back(pair<int, int>(-1,-1));
		return -AB_value(next_b, neuralnet, disks, depth - 1, -bet, -alp, cut_ratio, moves);
	}
	if (4 <= depth && disks <= 54 && move_number > 1) {
	...
	vector<pair<int, double> > next_b_v(0);
	vector<pair<int, int> > moves_chosen(moves);
	for (auto &next_b : next_bs) {
		vector<pair<int, int> > moves_route(moves);
		moves_route.emplace_back(moved_location(b ,next_b));
		double value = -AB_value(next_b, neuralnet, disks + 1, depth - 1, -bet, -alp, cut_ratio, moves_route);
		if (value > alp) {
			moves_chosen = moves_route;
			alp = value;
		}
		if (alp >= bet) {
			moves = moves_chosen;
...
double AB_move(Board &b, const Neuralnet &neuralnet, const int disks, const int depth, const double random, const double cut_ratio)
{
	int inputs(neuralnet.nodes[0]);
	vector<Board> next_bs = next_boards(b);
	int move_number(next_bs.size());
	...
	if (random >= 100 || depth == 0 || disks == 4) {
		...
		double sum_rate(0);
		for (auto &next_b : next_bs) {
			double value = -AB_value(next_b, neuralnet, disks + 1, depth - 1, -99999, 99999, cut_ratio);
...
vector<Board> random_play_boards(const int number, const Neuralnet &neuralnet, const int bottom_disks, const int top_disks, const int search_depth, const double random, const double cut_ratio)
{
	int games(0);
	...
	while (games < number) {
		...
		int disks = 4;
		while (!end) {
			double v = AB_move(b, neuralnet, disks, search_depth, random, cut_ratio);
			b.calculate_disks();
