...
void Book::newdata(const int disks, const  int turn, const int(&s)[8][8], const double upper, const double lower)
{
	...
	int depth = disks - disks0;
	int n(Size[depth]);
	ReferTable[depth].push_back(m2);
	//ReferTable[depth][n].resize(2);
	//ReferTable[depth][n][0] = m[0];
	//ReferTable[depth][n][1] = m[1];
	TransTableUpper[depth].push_back(upper);
	TransTableLower[depth].push_back(lower);
	for (int i(0); i < 2;++i) {
	...
	for (int i(0);i < sized;++i) {
		...
		if (ifs2) {
			...
			while (getline(ifs2, str)) {
				...
				TransTableLower[i].push_back(stod(token));
				getline(stream, token, ',');
				//TransTableUpper[i][n] = stod(token);
				TransTableUpper[i].push_back(stod(token));
				++n;
			}
			ifs2.close();
		}
