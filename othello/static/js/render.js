(function(global) {
  ...
  //-------------------------------------
  // var length = Math.min($(".contents").width(), $(".contents").height());
  // console.log(length);
  var length = document.getElementById("contents").clientWidth;
  // var length = 500;
  var COL = 8;
  var RECT_CANV = {
    x: 0,
  ...
  // $("#canv").attr("width", RECT_CANV.w);
  // $("#canv").attr("height", RECT_CANV.h);
  ...
  var COLOR_PANEL_4 = "#04cb04";
  var COLOR_PANEL_5 = "#03a803";
  var COLOR_PANEL_6 = "#006400";
  ...
  function render(ctx, state, point) {
    if (prev_revision < 0) {
      ...
      canv_cache.canv_pieaces = drawPutALL(state);;
  ...
  function drawPutALL(state) {
    ...
    for (var x = 0; x < COL; x++) {
      for (var y = 0; y < COL; y++) {
        if (state.putable[y * COL + x] != 0) {
          drawPut(ctx, x * CELL_SIZE, y * CELL_SIZE, state.turn);
        }
      }
    }
    return canv_cache.canv_pieaces;
  ...
  function drawPut(ctx, x, y, number) {
    var grad = ctx.createLinearGradient(x, y, x + CELL_SIZE, y + CELL_SIZE);
    var font_color;
    var fill_color;
    if (number > 0) {
      font_color = COLOR_BLACK;
      fill_color = COLOR_WHITE;
      grad.addColorStop(0, "rgb(160, 255, 160)");
      grad.addColorStop(0.8, fill_color);
      grad.addColorStop(1, fill_color);
    } else if (number < 0) {
      font_color = COLOR_WHITE;
      fill_color = COLOR_BLACK;
      grad.addColorStop(0, fill_color);
      grad.addColorStop(0.8, "rgb(70, 145, 70)");
      grad.addColorStop(1, "rgb(70, 70, 70)");
    }
  ...
  function drawEffect(state, point) {
    ...
    ctx.clearRect(0, 0, RECT_CANV.w, RECT_CANV.h);
    var i = x / CELL_SIZE + 8 * y / CELL_SIZE;
