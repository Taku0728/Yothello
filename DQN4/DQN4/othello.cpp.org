...
void sort_boards(map<string, pair<double, double> > book, vector<Board> &boards, const Neuralnet &neuralnet, const int disks, const int depth, const double cut_ratio) {
	vector<pair<int,double> > next_b_r(0);
	double sum_ratio(0);
	for (int i = 0; i != boards.size(); ++i) {
		vector<pair<int, int> > route(0);
		double value = -AB_value(book, boards[i], neuralnet, disks, depth, -99999, 99999, cut_ratio, route);
				...
				vector<pair<int, int> > &route)
{
	...
	if (move_number == 0) {
		...
		double value = -AB_value(book, next_b, neuralnet, disks, depth - 1, -bet, -alp, cut_ratio, route);
		return max(alp, value);
	}
	if (4 <= depth && disks <= 54 && move_number > 1) {
		int sub_depth = min(max(0, (depth - 8)/2*2), max(0, int((52 - depth)/2)*2));
		sort_boards(book, next_bs, neuralnet, disks + 1, sub_depth, cut_ratio);
	}
	...
	for (auto &next_b : next_bs) {
		...
		double t_value = -AB_value(book, next_b, neuralnet, disks + 1, depth - 1, -bet, -alp, cut_ratio, n_route);
				...
				const double cut_ratio)
{
	...
	if (random >= 100 || depth == 0 || disks == 4) {
		...
		for (auto &next_b : next_bs) {
			...
			value = -AB_value(book, next_b, neuralnet, disks + 1, depth - 1, -bet, -alp, cut_ratio);
			if (value > alp) {
				b = next_b;
				alp = value;
			}
		...
		for (auto &next_b : next_bs) {
			...
			if (info.first) {
				...
				value = -AB_value(book, next_b, neuralnet, disks + 1, depth - 1, -99999, 99999, cut_ratio);
			}
